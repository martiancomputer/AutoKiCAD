...
public:
    ...
    /**
     * Build the structured report that the JSON writer serialises.
     *
     * Separate from WriteJsonReport so callers that want the violations without a
     * file on disk -- the IPC API -- share one implementation with
     * `kicad-cli sch erc --format json` and cannot drift from it.
     */
    RC_JSON::ERC_REPORT BuildReport() const;
