...
bool DIFF_PAIR_PLACER::tryWalkDp( NODE* aNode, DIFF_PAIR &aPair, bool aSolidsOnly )
{
    ...
    // Test whether a walk actually succeeded, not whether the sentinel score is
    // positive. bestScore starts at 1e14, so `bestScore > 0.0` held even when all
    // four attempts failed: the caller was handed a default-constructed (empty)
    // DIFF_PAIR along with a success return. Downstream that surfaces as
    // DIFF_PAIR_PLACER::FixRoute() rejecting the route on its
    // CP()/CN() SegmentCount() < 1 guard while m_fitOk is true -- an
    // unroutable pair reported as a fit.
    if( found )
    {
