...
public:
     ...
     *
     * Split out of WriteJsonReport() so that consumers which want the report as
     * data rather than as a file on disk (notably the IPC API's RunDrc handler)
     * share one implementation with the file writers, instead of duplicating the
     * provider walk and severity resolution.
     */
    RC_JSON::DRC_REPORT BuildReport() const;
