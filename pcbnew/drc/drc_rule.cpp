...
wxString DRC_RULE::FormatRuleFromProto( const kiapi::board::CustomRule& aRule, wxString* aErrorText )
{
    ...
    auto getConstraintToken =
                ...
                bool* aAddWithinDiffPairs ) -> std::optional<wxString>
            {
                ...
                // Net-chain constraints. The .kicad_dru keywords are unprefixed
                // ("stub_length", not "net_chain_stub_length") and the parser has
                // accepted them all along; only this proto -> rule-text mapping
                // was missing, so SetCustomDesignRules answered CRS_INVALID for
                // constraints DRC fully implements.
                //
                // CRCT_NET_CHAIN_RETURN_PATH is deliberately absent: its rule form
                // is (constraint return_path (layer "B.Cu") (net "GND")), and
                // CustomRuleConstraint's oneof carries no layer+net variant to
                // serialise from. Adding it needs a proto change, not a case here.
                case kiapi::board::CRCT_NET_CHAIN_LENGTH:        return wxS( "net_chain_length" );
                case kiapi::board::CRCT_NET_CHAIN_STUB_LENGTH:   return wxS( "stub_length" );
