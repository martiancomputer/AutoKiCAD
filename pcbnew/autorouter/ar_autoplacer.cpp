...
{
    // The overlay is progress feedback, not part of the placement algorithm, and is
    // never set when there is no view to draw into -- both call sites already guard
    // m_progressReporter and m_refreshCallback the same way. Without this, running
    // the placer headlessly dereferences a null shared_ptr.
    if( !m_overlay )
        return;
