        ...
        API_HANDLER_BOARD( std::move( aContext ), aFrame )
{
    ...
    registerHandler<AutoplaceFootprints, AutoplaceFootprintsResponse>(
            &API_HANDLER_PCB::handleAutoplaceFootprints );
    registerHandler<SearchFootprints, SearchFootprintsResponse>(
            &API_HANDLER_PCB::handleSearchFootprints );
...
/**
 * Convert one report violation into its protobuf form.
 *
 * RC_JSON is the structure the DRC report writers already produce, so building
 * on it keeps the API and the `kicad-cli pcb drc --format json` output in step:
 * one implementation, one set of semantics.
 */
void packDrcViolation( const RC_JSON::VIOLATION& aSrc, EDA_UNITS aUnits, DrcViolation* aOut )
{
    ...
    if( sev == wxS( "error" ) )
        aOut->set_severity( types::RuleSeverity::RS_ERROR );
    else if( sev == wxS( "warning" ) )
        aOut->set_severity( types::RuleSeverity::RS_WARNING );
    else if( sev == wxS( "exclusion" ) )
        aOut->set_severity( types::RuleSeverity::RS_EXCLUSION );
    else if( sev == wxS( "info" ) )
        aOut->set_severity( types::RuleSeverity::RS_INFO );
    else if( sev == wxS( "ignore" ) )
        aOut->set_severity( types::RuleSeverity::RS_IGNORE );
    else
        aOut->set_severity( types::RuleSeverity::RS_UNKNOWN );

    for( const RC_JSON::AFFECTED_ITEM& item : aSrc.items )
    {
        DrcAffectedItem* outItem = aOut->add_items();
        outItem->mutable_id()->set_value( item.uuid.ToStdString() );
        outItem->set_description( item.description.ToStdString() );

        // RC_JSON carries coordinates as doubles already scaled to the report
        // units (the JSON schema publishes them that way), but common.types.Vector2
        // is int64 nanometers. Convert back to internal units rather than
        // truncating a millimetre value into a nanometre field.
        outItem->mutable_position()->set_x_nm(
                KiROUND( EDA_UNIT_UTILS::UI::FromUserUnit( pcbIUScale, aUnits, item.pos.x ) ) );
        outItem->mutable_position()->set_y_nm(
                KiROUND( EDA_UNIT_UTILS::UI::FromUserUnit( pcbIUScale, aUnits, item.pos.y ) ) );
    }
}


EDA_UNITS unitsFromProto( types::Units aUnits )
{
    switch( aUnits )
    {
    case types::Units::U_INCH:  return EDA_UNITS::INCH;
    case types::Units::U_MILS:  return EDA_UNITS::MILS;
    case types::Units::U_MM:    return EDA_UNITS::MM;
...
HANDLER_RESULT<RunDrcResponse> API_HANDLER_PCB::handleRunDrc( const HANDLER_CONTEXT<RunDrc>& aCtx )
{
    if( std::optional<ApiResponseStatus> busy = checkForBusy() )
        return tl::unexpected( *busy );
    ...
    if( !board )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( "no board is open" );
        return tl::unexpected( e );
    }

    const EDA_UNITS units = unitsFromProto( aCtx.Request.units() );

    // Severity mask. An empty request means the common case -- errors and
    // warnings, exclusions omitted -- rather than "report nothing".
    int severities = 0;
    ...
    else
    {
        ...
        {
            switch( static_cast<types::RuleSeverity>( severity ) )
            {
            case types::RuleSeverity::RS_ERROR:     severities |= RPT_SEVERITY_ERROR;     break;
            case types::RuleSeverity::RS_WARNING:   severities |= RPT_SEVERITY_WARNING;   break;
            case types::RuleSeverity::RS_EXCLUSION: severities |= RPT_SEVERITY_EXCLUSION; break;
            case types::RuleSeverity::RS_INFO:      severities |= RPT_SEVERITY_INFO;      break;
            default: break;
            }
        }
    }

    // Stale zone fills report connections that the filled board would make,
    // producing phantom unconnected items. Refill unless explicitly told not to.
    const bool refillZones = !aCtx.Request.refill_zones_set() || aCtx.Request.refill_zones();
    ...
    if( !drcEngine )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_UNHANDLED );
        e.set_error_message( "board has no DRC engine" );
        return tl::unexpected( e );
    }

    // Re-parse the rules before every run. The engine caches parsed rules from
    // whenever it was last initialised -- board load, for a long-lived
    // api-server session -- so custom rules written since (notably by
    // SetCustomDesignRules, which persists them to the .kicad_dru but does not
    // re-init) would otherwise be silently ignored. The DRC dialog does the same
    // thing before each run; the CLI jobs handler gets away without it only
    // because it loads the board fresh each time.
    try
    {
        drcEngine->InitEngine( wxFileName( board->GetDesignRulesPath() ) );
    }
    catch( const PARSE_ERROR& exc )
    {
        ...
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message(
                fmt::format( "custom design rules failed to parse: {}",
                             exc.What().ToStdString() ) );
        return tl::unexpected( e );
    }

    BOARD_COMMIT commit( mgr );
    ...
    try
    {
        drcEngine->RunTests( units, aCtx.Request.report_all_track_errors(),
                             aCtx.Request.check_schematic_parity() );
    }
    catch( const std::exception& exc )
    {
        ...
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_UNHANDLED );
        e.set_error_message( fmt::format( "DRC failed: {}", exc.what() ) );
        return tl::unexpected( e );
    }

    drcEngine->ClearViolationHandler();

    // Markers are board state, not an edit the user asked for, so keep them out
    // of undo and do not dirty the document.
    ...
    auto markersProvider = std::make_shared<DRC_ITEMS_PROVIDER>(
            board, MARKER_BASE::MARKER_DRC, MARKER_BASE::MARKER_DRAWING_SHEET );
    auto ratsnestProvider =
            std::make_shared<DRC_ITEMS_PROVIDER>( board, MARKER_BASE::MARKER_RATSNEST );
    auto parityProvider =
            std::make_shared<DRC_ITEMS_PROVIDER>( board, MARKER_BASE::MARKER_PARITY );

    markersProvider->SetSeverities( severities );
    ratsnestProvider->SetSeverities( severities );
    parityProvider->SetSeverities( severities );

    DRC_REPORT          reporter( board, units, markersProvider, ratsnestProvider, parityProvider );
    RC_JSON::DRC_REPORT report = reporter.BuildReport();

    RunDrcResponse response;

    for( const RC_JSON::VIOLATION& v : report.violations )
        packDrcViolation( v, units, response.add_violations() );

    for( const RC_JSON::VIOLATION& v : report.unconnected_items )
        packDrcViolation( v, units, response.add_unconnected_items() );

    for( const RC_JSON::VIOLATION& v : report.schematic_parity )
        packDrcViolation( v, units, response.add_schematic_parity() );

    for( const RC_JSON::IGNORED_CHECK& check : report.ignored_checks )
    {
        DrcIgnoredCheck* out = response.add_ignored_checks();
        out->set_key( check.key.ToStdString() );
        out->set_description( check.description.ToStdString() );
    }

    response.set_units( aCtx.Request.units() == types::Units::U_UNKNOWN ? types::Units::U_MM
                                                                       : aCtx.Request.units() );

    if( severities & RPT_SEVERITY_ERROR )
        response.add_included_severities( types::RuleSeverity::RS_ERROR );

    if( severities & RPT_SEVERITY_WARNING )
        response.add_included_severities( types::RuleSeverity::RS_WARNING );

    if( severities & RPT_SEVERITY_EXCLUSION )
        response.add_included_severities( types::RuleSeverity::RS_EXCLUSION );

    if( severities & RPT_SEVERITY_INFO )
        response.add_included_severities( types::RuleSeverity::RS_INFO );
...
HANDLER_RESULT<AutoplaceFootprintsResponse> API_HANDLER_PCB::handleAutoplaceFootprints(
        const HANDLER_CONTEXT<AutoplaceFootprints>& aCtx )
{
    if( std::optional<ApiResponseStatus> busy = checkForBusy() )
        return tl::unexpected( *busy );
    ...
    // AR_AUTOPLACER answers a bare AR_FAILURE when the board outline is degenerate,
    // which is by far the most likely way to call this wrongly. AUTOPLACE_TOOL checks
    // for it before invoking the placer and shows a specific message; do the same here
    // rather than returning an unexplained failure.
    ...
    if( bbox.GetWidth() == 0 || bbox.GetHeight() == 0 )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( fmt::format( "board edges must be defined on the {} layer",
                                          LayerName( Edge_Cuts ).ToStdString() ) );
        return tl::unexpected( e );
    }

    std::vector<FOOTPRINT*> footprints;

    if( aCtx.Request.footprints().empty() )
    {
        for( FOOTPRINT* fp : board->Footprints() )
    ...
    else
    {
        for( const types::KIID& id : aCtx.Request.footprints() )
        {
            ...
            if( !item || ( *item )->Type() != PCB_FOOTPRINT_T )
            {
                ApiResponseStatus e;
                e.set_status( ApiStatusCode::AS_BAD_REQUEST );
                e.set_error_message( fmt::format( "footprint with ID {} not found on the board",
                                                  id.value() ) );
                return tl::unexpected( e );
            }
    ...
    // AR_AUTOPLACER has no lock awareness at all; AUTOPLACE_TOOL strips locked
    // footprints before handing them over. Without this, a locked footprint would be
    // moved silently.
    if( !aCtx.Request.override_locks() )
    {
        std::erase_if( footprints,
                       [&]( FOOTPRINT* fp )
                       {
                           ...
                           // Only worth reporting for footprints the caller named; when
                           // placing the whole board, locked parts are the norm.
                           if( !aCtx.Request.footprints().empty() )
                               response.add_skipped_locked()->set_value( fp->m_Uuid.AsStdString() );
    ...
    if( footprints.empty() )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( "no placeable footprints: the board has none, or every named "
                             "footprint is locked and override_locks was not set" );
        return tl::unexpected( e );
    }

    std::map<KIID, std::pair<VECTOR2I, EDA_ANGLE>> before;

    for( FOOTPRINT* fp : footprints )
        before[fp->m_Uuid] = { fp->GetPosition(), fp->GetOrientation() };

    // Always a BOARD_COMMIT here; createCommit() picks the frame-bound form when a frame
    // exists, which is what makes the placement undoable in an attached editor.
    std::unique_ptr<COMMIT> commit = createCommit();
    AR_RESULT               result = AR_FAILURE;

    try
    {
        AR_AUTOPLACER autoplacer( board );
        result = autoplacer.AutoplaceFootprints( footprints,
                                                 static_cast<BOARD_COMMIT*>( commit.get() ),
                                                 aCtx.Request.place_offboard() );
    }
    catch( const std::exception& exc )
    {
        commit->Revert();
        ...
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_UNKNOWN );
        e.set_error_message( fmt::format( "autoplacement failed: {}", exc.what() ) );
        return tl::unexpected( e );
    }
    ...
    switch( result )
    {
    case AR_COMPLETED: response.set_result( APR_COMPLETED ); break;
    case AR_CANCELLED: response.set_result( APR_CANCELLED ); break;
    case AR_FAILURE:   response.set_result( APR_FAILURE );   break;
    default:           response.set_result( APR_UNKNOWN );   break;
    }

    // Reverted runs leave everything where it was, so the moved list is empty by
    // construction rather than by special case.
    for( FOOTPRINT* fp : footprints )
    {
        const auto& [pos, angle] = before.at( fp->m_Uuid );

        if( fp->GetPosition() != pos || fp->GetOrientation() != angle )
            response.add_moved()->set_value( fp->m_Uuid.AsStdString() );
    }
...
/// Default cap on SearchFootprints results. The stock libraries hold tens of thousands of
/// footprints, so an unbounded query would return a reply far larger than any caller wants.
static constexpr int MAX_FOOTPRINT_SEARCH_RESULTS = 250;


HANDLER_RESULT<SearchFootprintsResponse> API_HANDLER_PCB::handleSearchFootprints(
        const HANDLER_CONTEXT<SearchFootprints>& aCtx )
{
    if( std::optional<ApiResponseStatus> busy = checkForBusy() )
        return tl::unexpected( *busy );
    ...
    if( !adapter )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_UNKNOWN );
        e.set_error_message( "no footprint library adapter is available" );
        return tl::unexpected( e );
    }

    if( adapter->Rows().empty() )
    {
        // Distinct from "nothing matched": with no configured libraries there is nothing to
        // match against, and the caller's real problem is configuration.
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( "no footprint libraries are configured; check the global and "
                             "project footprint library tables" );
        return tl::unexpected( e );
    }
    ...
    for( const std::string& nickname : aCtx.Request.libraries() )
    {
        ...
        if( !adapter->HasLibrary( nick, false ) && !adapter->GetRow( nick ) )
        {
            ApiResponseStatus e;
            e.set_status( ApiStatusCode::AS_BAD_REQUEST );
            e.set_error_message( fmt::format( "no such footprint library: {}", nickname ) );
            return tl::unexpected( e );
        }

        wanted.push_back( nick );
    }

    FOOTPRINT_LIST_IMPL list;

    try
    {
        if( wanted.empty() )
        {
            // nullptr means every configured library. Cold, this reads them all; KiCad then
            // persists an fp-info-cache, so the cost is paid once.
            list.ReadFootprintFiles( adapter, nullptr, nullptr );
        }
        else
        {
            for( const wxString& nick : wanted )
                list.ReadFootprintFiles( adapter, &nick, nullptr );
        }
    }
    catch( const std::exception& exc )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_UNKNOWN );
        e.set_error_message( fmt::format( "footprint library scan failed: {}", exc.what() ) );
        return tl::unexpected( e );
    }
    ...
    int limit = aCtx.Request.max_results() > 0 ? (int) aCtx.Request.max_results()
                                               : MAX_FOOTPRINT_SEARCH_RESULTS;

    SearchFootprintsResponse response;
    int                      matched = 0;

    for( const std::unique_ptr<FOOTPRINT_INFO>& info : list.GetList() )
    {
        if( !info )
            continue;

        // Description and keywords are what make a search useful, and both are lazily
        // loaded; GetDesc()/GetKeywords() fault them in.
        //
        // Matched per field rather than against one concatenated string: joining them
        // lets a query straddle a boundary, so "0603 resistor" matched
        // Resistor_SMD:R_Array_Concave_2x0603 on the seam between its name and its
        // library nickname. A term must appear within a single field.
        const wxString fields[] = { info->GetFootprintName().Lower(),
                                    info->GetLibNickname().Lower(),
                                    info->GetDesc().Lower(),
                                    info->GetKeywords().Lower() };
        ...
        FootprintSearchResult* out = response.add_results();
        out->mutable_id()->set_library_nickname( info->GetLibNickname().ToStdString() );
        out->mutable_id()->set_entry_name( info->GetFootprintName().ToStdString() );
        out->set_description( info->GetDesc().ToStdString() );
        out->set_keywords( info->GetKeywords().ToStdString() );
        out->set_pad_count( info->GetUniquePadCount() );
    }

    response.set_total_indexed( (uint32_t) list.GetCount() );

    if( list.GetErrorCount() )
    {
        // Surfaced rather than dropped: a library that failed to read makes the result list
        // silently short, which is indistinguishable from a correct empty answer.
        response.add_errors( list.GetErrorMessages().ToStdString() );
    }
